...
export default function Cart() {
    ...
    return (
      <Layout title="Panier" hideNav>
        <div className="flex flex-col items-center justify-center flex-1 py-20 px-4 text-center h-[calc(100vh-64px)]">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Panier dyalk khawi</h2>
          <p className="text-gray-500 max-w-sm mx-auto mb-8">Mazal ma zedti hata sneaker. Bda daba w khtar li bghiti.</p>
          <Button 
            size="lg" 
  ...
  return (
    <Layout title={`Panier (${itemCount})`} hideNav>
      <div className="flex flex-col lg:flex-row gap-8 px-4 md:px-8 py-6 w-full pb-32 lg:pb-12">
                  ...
                  <div className="flex flex-1 flex-col justify-between py-1">
                    <div className="flex justify-between items-start gap-2">
                      <div>
                        ...
                        <div className="text-sm text-gray-500 mt-1">Pointure {item.size}</div>
                      </div>
                      <div className="font-bold text-gray-900 shrink-0">${(item.price * item.quantity).toFixed(2)}</div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center bg-gray-50 rounded-full border border-gray-200">
                        <button 
                          ...
                          aria-label={`Reduire quantite pour ${item.productName}`}
                        ...
                        <button 
                          onClick={() => updateQuantity(item.productId, item.size, item.quantity + 1)}
                          aria-label={`Augmenter quantite pour ${item.productName}`}
                          className="w-8 h-8 flex items-center justify-center text-gray-500 hover:text-gray-900"
                        >
        ...
        <div className="w-full lg:w-96 shrink-0">
          <div className="bg-gray-900 rounded-3xl p-6 text-white sticky top-24">
            <h3 className="text-xl font-bold mb-6">Khllasa dyal commande</h3>
