...
export function Layout({ children, hideNav = false, backButton = false, title }: { children: ReactNode; hideNav?: boolean; backButton?: boolean; title?: string }) {
  ...
  return (
    // Admin pages override the global dir="rtl" back to LTR
    <div className="min-h-dvh flex flex-col bg-gray-50 pb-16 md:pb-0" dir={isAdmin ? "ltr" : undefined}>
      <a
        href="#main-content"
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100 flex-none h-14 md:h-16 flex items-center justify-between px-4 md:px-8" role="banner">
        <div className="flex items-center gap-3">
          {backButton ? (
            <button
              ...
              className="p-2 -ms-2 rounded-full hover:bg-gray-100 active:scale-95 transition-transform text-gray-900"
            >
              {/* ArrowLeft flips to point right in RTL via rtl-flip — correct "back" direction */}
        ...
        <div className="hidden items-center gap-4 md:flex">
          {!isAdmin && (
            <nav className="flex items-center gap-6 me-4" aria-label="التنقل الرئيسي">
              <Link href="/" className={`text-sm font-semibold ${location === "/" ? "text-red-600" : "text-gray-600 hover:text-gray-900"}`}>الرئيسية</Link>
              <Link href="/products" className={`text-sm font-semibold ${location === "/products" ? "text-red-600" : "text-gray-600 hover:text-gray-900"}`}>المتجر</Link>
            </nav>
          )}
          ...
          <Link
            ...
            className="relative p-2 rounded-full hover:bg-gray-100 active:scale-95 transition-transform text-gray-900"
          >
        ...
        <div className="flex md:hidden">
          <Link
            ...
            className="relative p-2 -me-2 rounded-full hover:bg-gray-100 active:scale-95 transition-transform text-gray-900"
          >
            <ShoppingBag className="w-5 h-5" />
            <AnimatePresence>
              {itemCount > 0 && (
      ...
      {!hideNav && (
        <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-gray-100 flex items-center justify-around px-4 z-50 pb-safe" aria-label={isAdmin ? "Mobile navigation" : "التنقل السفلي"}>
          <Link href="/" className={`flex flex-col items-center gap-1 p-2 w-16 ${location === "/" ? "text-red-600" : "text-gray-400 hover:text-gray-900"}`}>
            ...
            <span className="text-[10px] font-semibold">{isAdmin ? "Home" : "الرئيسية"}</span>
          </Link>
          <Link href="/products" className={`flex flex-col items-center gap-1 p-2 w-16 ${location === "/products" ? "text-red-600" : "text-gray-400 hover:text-gray-900"}`}>
            ...
            <span className="text-[10px] font-semibold">{isAdmin ? "Store" : "المتجر"}</span>
          </Link>
          <Link href="/cart" className={`flex flex-col items-center gap-1 p-2 w-16 relative ${location === "/cart" ? "text-red-600" : "text-gray-400 hover:text-gray-900"}`}>
            ...
            <span className="text-[10px] font-semibold">{isAdmin ? "Cart" : "السلة"}</span>
          </Link>
        </nav>
      )}
    </div>
