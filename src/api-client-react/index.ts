...
function mapOrder(row: OrderRow): Order {
	const items = Array.isArray(row.items) ? row.items.map((item) => mapOrderItem(item)) : [];
...
async function listProducts(params?: ListProductsParams): Promise<Product[]> {
	...
	if (params?.category) {
		...
		products = products.filter((product) => slugify(product.category) === requestedCategory);
	}
...
async function listCategories(): Promise<Category[]> {
	...
	return (data ?? []).map((row, index) => mapCategory(row as CategoryRow, index + 1));
}
...
async function updateProduct(id: number, body: CreateProductBody): Promise<Product> {
	ensureSupabaseConfigured();
	await ensureAdminSession();
...
async function createOrder(body: CreateOrderBody): Promise<Order> {
	...
	const delivery = 5;
	const hasDiscount = Boolean(body.discountCode && body.discountCode.trim() !== "");
...
async function updateOrderStatus(id: number, body: UpdateOrderStatusBody): Promise<Order> {
	ensureSupabaseConfigured();
	await ensureAdminSession();
...
async function getDashboardStats(): Promise<DashboardStats> {
	...
	if (ordersResult.error) {
		throw new Error(`Failed to load dashboard stats: ${ordersResult.error.message}`);
	}

	if (productsResult.error) {
		throw new Error(`Failed to count products: ${productsResult.error.message}`);
	}
...
export function useHealthCheck(
	...
	return useTypedQuery(getHealthCheckQueryKey(), async () => ({ status: "ok" }), options);
}
...
export function useListProducts(
	...
	return useTypedQuery(getListProductsQueryKey(params), () => listProducts(params), options);
}
...
export function useGetFeaturedProducts(
	...
	return useTypedQuery(getGetFeaturedProductsQueryKey(), getFeaturedProducts, options);
}

export const getGetProductQueryKey = (id: number) => ["products", "detail", id] as const;

export function useGetProduct(
	id: number,
	options?: QueryHookOptions<Product>,
): UseQueryResult<Product, Error> & { queryKey: QueryKey } {
	return useTypedQuery(getGetProductQueryKey(id), () => getProduct(id), options);
}
...
export function useUpdateProduct<TContext = unknown>(
	options?: MutationHookOptions<Product, { id: number; data: CreateProductBody }, TContext>,
): UseMutationResult<Product, Error, { id: number; data: CreateProductBody }, TContext> {
	return useMutation({
...
export function useListOrders(
	...
	return useTypedQuery(getListOrdersQueryKey(params), () => listOrders(params), options);
}

export const getGetOrderQueryKey = (id: number) => ["orders", "detail", id] as const;
...
export function useUpdateOrderStatus<TContext = unknown>(
	options?: MutationHookOptions<Order, { id: number; data: UpdateOrderStatusBody }, TContext>,
): UseMutationResult<Order, Error, { id: number; data: UpdateOrderStatusBody }, TContext> {
	return useMutation({
...
export const getGetDashboardStatsQueryKey = () => ["stats", "dashboard"] as const;

export function useGetDashboardStats(
	options?: QueryHookOptions<DashboardStats>,
): UseQueryResult<DashboardStats, Error> & { queryKey: QueryKey } {
	return useTypedQuery(getGetDashboardStatsQueryKey(), getDashboardStats, options);
}

export const getGetRecentOrdersQueryKey = () => ["stats", "recent-orders"] as const;
